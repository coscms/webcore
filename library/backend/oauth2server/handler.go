...
func makeLoginHandler(login func(echo.Context) error) func(w http.ResponseWriter, r *http.Request) {
...
func authHandler(w http.ResponseWriter, r *http.Request) {
	...
	var err error
	if !ok || user == nil {
		err = ctx.Redirect(backend.URLFor(RoutePrefix + `/login`))
		if err != nil {
			http.Error(w, err.Error(), http.StatusBadRequest)
		}
		return
	...
	if agreed {
		err = ctx.Redirect(backend.URLFor(RoutePrefix + `/authorize`))
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
		}
	}
...
func makeLogoutHandler(logout func(echo.Context) error) func(w http.ResponseWriter, r *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		ctx := defaults.MustGetContext(r.Context())
		err := logout(ctx)
		if err != nil {
