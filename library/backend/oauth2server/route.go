...
func Route(g echo.RouteRegister, loginHandler func(echo.Context) error, logoutHandler func(echo.Context) error) {
	...
	g.Route(`GET,POST`, `/login`, makeLoginHandler(loginHandler), middleware.AuthCheck).SetMetaKV(route.PermGuestKV())    // (step.2) 登录页面
	g.Route(`GET,POST`, `/auth`, authHandler, middleware.AuthCheck).SetMetaKV(route.PermPublicKV())                       // (step.3) 授权页面
	g.Route(`GET,POST`, `/logout`, makeLogoutHandler(logoutHandler), middleware.AuthCheck).SetMetaKV(route.PermGuestKV()) // 退出登录
