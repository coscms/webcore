...
type Slice_NgingCloudStorage = factory.Slicex[*NgingCloudStorage]
...
func (a *NgingCloudStorage) CtxFrom(source factory.Model) factory.Model {
...
func (a *NgingCloudStorage) List(recv interface{}, mw func(db.Result) db.Result, page, size int, args ...interface{}) (func() int64, error) {
	...
	if err == nil {
		...
		case *[]*NgingCloudStorage:
			err = a.base.FireReaded(a, queryParam, Slice_NgingCloudStorage(*v))
		case []*NgingCloudStorage:
			err = a.base.FireReaded(a, queryParam, Slice_NgingCloudStorage(v))
		case factory.Ranger:
...
func (a *NgingCloudStorage) GetDiffColumns(old *NgingCloudStorage) (changedCols []interface{}) {
	...
	if old.Secret != a.Secret {
		changedCols = append(changedCols, `secret`)
	}

	if old.Bucket != a.Bucket {
		changedCols = append(changedCols, `bucket`)
	}

	if old.Endpoint != a.Endpoint {
		changedCols = append(changedCols, `endpoint`)
	}
	...
	if old.Secure != a.Secure {
		changedCols = append(changedCols, `secure`)
	}

	if old.Baseurl != a.Baseurl {
		changedCols = append(changedCols, `baseurl`)
	}
...
func (a *NgingCloudStorage) Save(old *NgingCloudStorage, args ...interface{}) (affected int64, err error) {
	a.Updated = uint(time.Now().Unix())
	if len(a.Type) == 0 {
		a.Type = "aws"
	}
	...
	if old == nil {
		old = NewNgingCloudStorage(a.Context())
		old.CtxFrom(a)
...
func (a *NgingCloudStorage) Clone() *NgingCloudStorage {
	cloned := NgingCloudStorage{Id: a.Id, Name: a.Name, Type: a.Type, Key: a.Key, Secret: a.Secret, Bucket: a.Bucket, Endpoint: a.Endpoint, Region: a.Region, Secure: a.Secure, Baseurl: a.Baseurl, Created: a.Created, Updated: a.Updated}
	cloned.CtxFrom(a)
	return &cloned
}
...
func (a *NgingCloudStorage) Validate(column string, value interface{}) error {
	return a.base.Validate(a, column, value)
}

func (a *NgingCloudStorage) TrimOverflowText(column string, value string) string {
